...
import com.tianze.bean.CanPackageBean;
...
public class CanUtils {
     ...
     * @return
     */
    public Map<String, Object> parsePackage(ByteBuf bf, String canCode, int packageIdLen, String vinCode) {
        ...
        while (bf.readableBytes() > 0) {
            ...
            if (bean == null) {
                logger.error("终端[{}], 软件版本号[{}]，不存在CAN包ID[{}]", vinCode, canCode, packageIdStr);
                return fieldVals;
            }
            int packageParamsLen = bean.getPackageParamsLen();
            //byte[] packageLenByte = new byte[packageParamsLen];
     ...
     * @return
     */
    public Map<String, Object> parsePackage(byte[] packages, String canCode, int packageIdLen, String vinCode) {
        return this.parsePackage(Unpooled.wrappedBuffer(packages), canCode, packageIdLen, vinCode);
    }
